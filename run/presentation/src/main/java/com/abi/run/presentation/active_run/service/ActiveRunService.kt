package com.abi.run.presentation.active_run.service
...
class ActiveRunService: Service() {
    ...
    private val baseNotification by lazy {
        NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            ...
            .setContentTitle(getString(com.abi.run.presentation.R.string.active_run))
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                start(activityClass = Class.forName(activityClass))
            }
            ACTION_STOP -> {
                stop()
            }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= 26) {
            val channel = NotificationChannel(
                CHANNEL_ID,
                getString(com.abi.run.presentation.R.string.active_run),
